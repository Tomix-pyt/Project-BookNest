(function () {
  ...
  document.querySelectorAll('.rail-btn[data-view]').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      if (v === 'library') loadLibrary('all');
      else if (v === 'recent') loadRecent();
      else search(el.input.value.trim());
    });
  });

  el.theme.addEventListener('click', function () {
    document.body.classList.toggle('dark');
    write(THEME_KEY, document.body.classList.contains('dark') ? 'dark' : 'light');
  });

  function updateOnline() { el.offline.hidden = navigator.onLine; }
  window.addEventListener('online', updateOnline);
  window.addEventListener('offline', updateOnline);

  /* ---------- init ---------- */
  try { if (localStorage.getItem(THEME_KEY) === '"dark"' || localStorage.getItem(THEME_KEY) === 'dark') document.body.classList.add('dark'); } catch (e) {}
  updateOnline();
  search('', true); // true = force a fresh fetch on real page load only
